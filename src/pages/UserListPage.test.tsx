...
test("renders User List page with functionality", async () => {
  ...
  const leanneGrahamRow = await screen.findByText(/Leanne Graham/i);
  expect(leanneGrahamRow).toBeInTheDocument();
  ...
  // const searchInput = screen.getByPlaceholderText(/Enter name to search/i);
  ...
  userEvent.click(within(nameHeader).getByRole("button"));
  // Wait for the sorting to take effect
  await waitFor(() => {
    // Check if the table is sorted in descending order
    const firstRow = screen.getByLabelText("user-row-0");
    expect(within(firstRow).getByText("Chelsey Dietrich")).toBeInTheDocument();
  });
});
